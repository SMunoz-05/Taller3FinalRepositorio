...
public class PlayerMovement : MonoBehaviour
{
    ...
    public float iceFriction = 0.05f;  // resbaloso
    ...
    void Update()
    {
        ...
        // Guardamos dirección de movimiento (para deslizar)
        if (move.magnitude > 0.1f)
            lastMoveDir = move;

        // Deslizamiento si estamos sobre hielo
        if (currentFriction < 0.5f && move.magnitude < 0.1f)
            move = lastMoveDir * (1 - currentFriction);
    ...
    {
        // Detecta si toca un objeto con etiqueta "Ice"
